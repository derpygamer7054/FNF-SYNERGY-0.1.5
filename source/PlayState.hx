				...
				switch(achievementName)
				{
					case 'derp_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'derp_nomiss':
									if(achievementName == 'derp_nomiss') unlock = true;
							}
						}
				    case 'week1':
						if(isStoryMode && campaignMisses + songMisses < 10000000000 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'week1':
									if(achievementName == 'week1') unlock = true;
							}
						}
					case 'madlad':
						if(Paths.formatToSongPath(SONG.song) == 'haywire-expert' && !usedPractice) {
							unlock = true;
						}
					case 'Cracked':
